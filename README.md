...
(`git rebase -i HEAD~2`): два коммита объединены в один — `d809cb9`
`chore: dummy commit 1`. Временный файл `rebase_test.txt` из той демонстрации
удалён из репозитория как не несущий пользы; доказательством задания служат
`before_rebase.txt` / `after_rebase.txt`.
